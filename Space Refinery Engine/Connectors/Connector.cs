		...
		public virtual void DeserializeState(XmlReader reader, SerializationData serializationData, SerializationReferenceHandler referenceHandler)
		{
			lock (SyncRoot)
			{
				reader.ReadStartElement(nameof(Connector));
				{
					...
					if (hasA)
					{
						reader.DeserializeReference<IConnectable>(referenceHandler, (es) => a = es, $"{nameof(connectables.connectableA)}_GUID");
					}

					if (hasB)
					{
						reader.DeserializeReference<IConnectable>(referenceHandler, (es) => b = es, $"{nameof(connectables.connectableB)}_GUID");
					}
