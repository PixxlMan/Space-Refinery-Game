	...
	public struct ResourceUnitData : IUIInspectable, IEquatable<ResourceUnit>
	{
			...
			foreach (var (resourceType, SMR) in resourceAndSMRs)
			{
				resourceUnitDataDictionary.Add(
					resourceType,
						new(
							resourceType,
							(MolesUnit)((DN)(Moles * SMR) / currentSubstanceAmountRatio), // Distribute moles according to SMR and substance amount ratio (Sorry, I don't know what that actually means either - it was like this when I came! Promise.).
							(EnergyUnit)((DN)InternalEnergy / resourceAndSMRs.Length))); // Evenly distribute energy.
			}
		...
		public static void DoCreation(ChemicalType selected, ref ResourceUnitData newResourceUnit)
		{
			ImGui.Indent();
			{
				...
				float massFloat = ((DN)newResourceUnit.Mass).ToFloat();
		...
		public static ResourceUnitData Deserialize(XmlReader reader)
		{
			...
			chemicalType = ChemicalType.GetChemicalType(reader.ReadString(nameof(Space_Refinery_Engine.ChemicalType.ChemicalName)));
